...
MACROPHAGE_ACTIVATED = 'activated_macrophage'
TCELL = 't_cell'

# Load keys
LOAD_FAST_BACTERIA = 'load_fast_bacteria'
LOAD_SLOW_BACTERIA = 'load_slow_bacteria'
LOAD_MACROPHAGES_PER_BPS = 'macs_per_bps'
LOAD_MACROPHAGES_PER_LYMPH = 'macs_per_lymph'
...
P_DEATH_ACTIVATED_MACROPHAGE = 'prob_death_act_mac'
...
P_ACTIVATED_MACROPHAGE_INGEST_FAST = 'prob_act_mac_ingest_fast'
P_ACTIVATED_MACROPHAGE_INGEST_SLOW = 'prob_act_mac_ingest_slow'
P_MIGRATE_REGULAR_MACROPHAGE = 'prob_reg_mac_migrate'
P_MIGRATE_INFECTED_MACROPHAGE = 'prob_inf_mac_migrate'
P_MIGRATE_ACTIVATED_MACROPHAGE = 'prob_act_mac_migrate'
P_TCELL_RECRUITMENT = 'prob_tcell_recruit'
P_TCELL_MIGRATION = 'prob_tcell_migrate'
P_TCELL_DEATH = 'prob_tcell_death'
P_MACROPHAGE_ACTIVATION = 'prob_macrophage_activation'


class TB_FSIcRInAT(LungLymph):
    ...
    def __init__(self, positions, parameters, weight_method=HORSFIELD):
        ...
        expected_parameters = [LOAD_FAST_BACTERIA, LOAD_SLOW_BACTERIA,
                               LOAD_MACROPHAGES_PER_BPS, LOAD_MACROPHAGES_PER_LYMPH,
                               ...
                               P_DEATH_REGULAR_MACROPHAGE, P_DEATH_INFECTED_MACROPHAGE, P_DEATH_ACTIVATED_MACROPHAGE,
                               ...
                               P_ACTIVATED_MACROPHAGE_INGEST_FAST, P_ACTIVATED_MACROPHAGE_INGEST_SLOW,
                               P_MIGRATE_REGULAR_MACROPHAGE, P_MIGRATE_INFECTED_MACROPHAGE,
                               P_MIGRATE_ACTIVATED_MACROPHAGE,
                               P_TCELL_RECRUITMENT, P_TCELL_MIGRATION, P_TCELL_DEATH,
                               P_MACROPHAGE_ACTIVATION]
        ...
        for node in range(0, 36):
            loads[node] = dict()
            loads[node][MACROPHAGE_REGULAR] = self.parameters[LOAD_MACROPHAGES_PER_BPS]

        for node in range(36, 45):
            loads[node] = dict()
            loads[node][MACROPHAGE_REGULAR] = self.parameters[LOAD_MACROPHAGES_PER_LYMPH]
        ...
        # Load bacteria
        fast_to_load = self.parameters[LOAD_FAST_BACTERIA]
        slow_to_load = self.parameters[LOAD_SLOW_BACTERIA]
        ...
        self.total_fast_bac = self.total_slow_bac = self.total_intracellular_bac = \
            ...
            self.total_inf_macrophage_and_fast = self.total_inf_macrophage_and_slow = \
            self.total_tcell_recruit = self.total_tcell_migration = self.total_tcell = 0.0
    ...
    def update_totals(self):
        ...
        for node in self.node_list_bps:
            ...
            self.total_inf_macrophage_and_slow += node.subpopulations[MACROPHAGE_INFECTED] * \
                                                  node.subpopulations[BACTERIA_SLOW]
            self.total_tcell_migration += node.subpopulations[TCELL] * lymph_degree
        ...
        for node in self.node_list_ln:
            ...
            self.total_inf_macrophage_and_slow += node.subpopulations[MACROPHAGE_INFECTED] * \
                                                  node.subpopulations[BACTERIA_SLOW]
            self.total_tcell_recruit += node.subpopulations[MACROPHAGE_INFECTED]
            self.total_tcell_migration += node.subpopulations[TCELL] * lymph_degree
            self.total_tcell += node.subpopulations[TCELL]
    ...
    def events(self):
        ...
        # TCell recruitment
        events.append((self.parameters[P_TCELL_RECRUITMENT] * self.total_tcell_recruit, lambda f: self.recruit_tcell()))

        # TCell migration
        events.append((self.parameters[P_TCELL_MIGRATION] * self.total_tcell_migration, lambda f: self.migrate_tcell()))

        # TCell death
        events.append((self.parameters[P_TCELL_DEATH] * self.total_tcell, lambda f: self.death_tcell()))
    ...
    def recruit_tcell(self):

        r = np.random.random() * self.total_tcell_recruit
    ...
    def migrate_tcell(self):
        ...
        for node in self.node_list.values():
            neighbouring_lymph_edges = self.get_neighbouring_edges(node, LYMPHATIC_VESSEL)
            running_total += node.subpopulations[TCELL] * len(neighbouring_lymph_edges)
            if running_total > r:
                # Choose a lymph edge
                neighbour_index = np.random.randint(0, len(neighbouring_lymph_edges))
                neighbour = neighbouring_lymph_edges[neighbour_index][0]
                neighbour.update(TCELL, 1)
                node.update(TCELL, -1)
                return

    def death_tcell(self):

        r = np.random.random() * self.total_tcell

        running_total = 0
        for node in self.node_list.values():
            running_total += node.subpopulations[TCELL]
            if running_total > r:
                node.update(TCELL, -1)
                return
