...
class MetapopulationNetwork(nx.Graph):
    ...
    def __init__(self, nodes, edges, species_keys, events):
        ...
        self.node_list = []
        # TODO - assert node list in ID order?
        for node in nodes:
        ...
        for (node1, node2, edge_dict) in edges:
            assert node1 in self.node_list, "Must supply valid node for edge: {0}".format(node1)
            assert node2 in self.node_list, "Must supply valid node for edge: {0}".format(node2)
    ...
    def run(self, time_limit):
        ...
        while self.time < time_limit:
            ...
            for event in self.events:
                event.total = 0
            # Process each node, updating the total for each event
            for node in self.node_list:
                for event in self.events:
                    event.update_total_for_node(node)

            # Events now have updated totals
            # Pick an event and a time (dt) for it to occur
            total = sum([event.get_rate() for event in self.events])
            if total == 0.0:
                self.timestep_output()
