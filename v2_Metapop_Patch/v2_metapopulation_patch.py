...
class MetapopulationPatchNetwork(nx.Graph):
...
class LungMetapopulationNetwork(MetapopulationPatchNetwork):
    ...
    def __init__(self, p_transmit, p_growth, time_limit, initial_loads):
        ...
        pos[34] = (8.5, 4.25)
        pos[35] = (8.5, 3)
        MetapopulationPatchNetwork.__init__(self, 36, edges, p_transmit, p_growth, time_limit, initial_loads, pos)


if __name__ == '__main__':
    pos = {0: (0, 0), 1: (5, 5), 2: (0, 5)}
    init = {1: 10}
    mpn = MetapopulationPatchNetwork(3, [(0, 1), (0, 2), (1, 2)], 0.2, 0.1, 10, init, node_positions=pos)
    mpn.run()
    mpn.display("T")
    mpn.movie("movie",100)