...
class Patch:
    ...
    def __init__(self, patch_id, counts, attributes, position=(0, 0)):
        self.id = patch_id
...
class CompartmentalMetapopulationNetwork(nx.Graph):
    ...
    def __init__(self, node_count, edges, time_limit, compartments, initial_loads, patch_attributes,
                 node_positions=None):
        ...
        for node_id in range(node_count):
            ...
            for compartment in compartments:
                if node_id in initial_loads and compartment in initial_loads[node_id]:
                    counts[compartment] = initial_loads[node_id][compartment]
                else:
                    counts[compartment] = 0
            # Create patch
            p = Patch(node_id, counts, patch_attributes[node_id], position)
            self.add_node(p)
